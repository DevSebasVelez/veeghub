...
export default async function ReportsPage({
  ...
  const months = Array.from({ length: 12 }, (_, index) => startOfMonth(subMonths(new Date(), 11 - index)));
  const monthOptions = months.map((date) => ({
    value: format(date, "yyyy-MM"),
    label: format(date, "MMMM yyyy", { locale: es }),
  }));
  const selectedMonth = month ?? monthOptions.at(-1)?.value ?? format(new Date(), "yyyy-MM");
  ...
  const [invoices, payments, receivables, selectedInvoices, selectedPayments, pendingReceivables] =
    await Promise.all([
  ...
  const monthlyData = months.map((date) => {
    ...
    const monthPayments = payments.filter((payment) => format(payment.paidAt, "yyyy-MM") === key);
    const monthReceivables = receivables.filter((item) => format(item.createdAt, "yyyy-MM") === key);
    const invoiced = monthInvoices.reduce((sum, invoice) => sum + Number(invoice.total), 0);
  ...
  const selectedInvoiced = selectedInvoices.reduce((sum, invoice) => sum + Number(invoice.total), 0);
  const selectedCollected = selectedPayments.reduce((sum, payment) => sum + Number(payment.amount), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <Metric label="Facturado en el mes" value={formatCurrency(selectedInvoiced)} tone="blue" />
        <Metric label="Cobrado en el mes" value={formatCurrency(selectedCollected)} tone="emerald" />
        <Metric label="Total por cobrar" value={formatCurrency(totalPending)} tone="amber" />
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card className="rounded-lg">
          <CardHeader>
            ...
            <CardDescription>Comparativo mensual de los últimos 12 meses.</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="rounded-lg">
          <CardHeader>
            ...
            <CardDescription>Cobros recibidos contra pendiente generado.</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card className="rounded-lg">
          <CardHeader>
            ...
            <CardDescription>Emitidas en el periodo seleccionado.</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
        ...
        <Card className="rounded-lg">
          <CardHeader>
            ...
            <CardDescription>Pagos registrados en el periodo seleccionado.</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {selectedPayments.map((payment) => (
                  <TableRow key={payment.id}>
                    <TableCell>{payment.receivable.title}</TableCell>
